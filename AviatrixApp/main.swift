...
func fly(myPlane : Aviatrix) {
    ...
    for (index, city) in destinations.enumerated() {
        let distance = myPlane.distanceTo(target: city, currentLocation: plane.location)
...
func refuel(myPlane : Aviatrix) {
    ...
    print("Refueling...")
    print("⛽ Here in \(plane.location), jet fuel costs $\(AviatrixData().fuelPrices[plane.location]!)/gallon")
...
var plane = Aviatrix(authorName: "☁︎ Isabella Hochschild ☁︎", planeLocation: "St. Louis")
...
while command != "q" {
    ...
    if command == "a" {
        ...
        print("⚠️Please enter a valid command⚠️")
//        print("⚠️ please enter a valid command ⚠️")
    }
}
